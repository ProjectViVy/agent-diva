...
9. **Evolution 只管理 Skill 演进。** SOP 是特殊 Skill、Skill 前置产物还是独立可复用资产，
   ...
   状态机。旧 AutoDream → SOP/Skill 链路不再修补。AutoDream 必须整理 STM（直写），
   并按 P19 允许表给 Laputa 生成待审人格提案；旧实现未通过。2026-08-14 已跑独立
   测试，STM 路径仍测不到。诊断另轨，不得写红线/梦/用户偏好/BML/Skill。
...
状态：`Approved / Implementation Pending`（用户 2026-08-15「点头」；同日「切」）。
保护分支：`protect/cognitive-pre-clean-break-20260815` @ `2aab18cc`。未改生产。
