...
2026-08-14 用户拍板：先全部走工具，不自动装配（吃过装配把事情搞复杂的教训）。
获取面要少：第一版 **一个读工具**，不要 CRUD 一串。

| 工具 | 作用 |
| --- | --- |
| `actmem`（唯一） | 读 Pulse、Work、某胶囊或目录；有界返回；**不搜 BML** |
| 已有 session 读取 | 要原文时读 transcript；不是 ACTMEM 工具 |

写入不靠聊天工具：发言即写 Pulse、空闲写胶囊、AutoDream 整理、用户在 Memory 页改。
系统提示一句：跨会话近事用 `actmem`，需要再读。禁止每轮强制 call。

代价（接受）：模型有时不调，会装失忆。「连发你好」只有去读才看得见。先试这条，再考虑加装配。
