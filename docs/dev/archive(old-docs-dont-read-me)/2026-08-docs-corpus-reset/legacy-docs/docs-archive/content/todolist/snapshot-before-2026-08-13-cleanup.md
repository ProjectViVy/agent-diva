# TODOLIST 历史快照（归档于 2026-08-13）

> 本文件是清理前的完整项目清单快照，包含已完成、已取消、已被取代、重复和当时仍
> 开放的记录。它只用于历史追溯，不是当前 backlog，也不得作为当前实施顺序或产品
> 决策依据。当前有效事项见仓库根目录 `TODOLIST.md`。
...
- [x] **Approval dual-channel unification + 超时 UX** `sev-P3`
  **2026-08-12 修复：** GUI 启动与消费均收敛到 `start_approval_stream` / `approval-event`；
  legacy `start_command_approval_stream`、注册和 capability 描述已删除。倒计时归零时
  仅触发一次详情刷新，批准按钮继续 fail closed，服务端 `expired` 才能成为终态；刷新
  失败保留 `0s`、禁用批准并提供既有恢复刷新入口。验证与 legacy 删除证明见
  `docs/logs/2026-08-code-review-residuals/`。
...
- [x] **CTX/工具链前端可读性（artifact ref）** `sev-P3`
  GUI 现在对严格 `ToolResultRef v1` 守卫展示 preview、规模、artifact ID、read hint，
  并提供真实 `artifact://` 引用复制动作；损坏 JSON、普通文本和错误结果保持原文本展示。
  验证见 `docs/logs/2026-08-code-review-residuals/`。
- [x] **Final-wire core hash 在无 cache_control 提供商上为空** `sev-P3`
  Agent 调用边界按 `ToolDefinitionSet.core_count` 计算显式 CORE hash；有 provider
  cache anchor 时仍保留 final-wire hash，DEFERRED 变化不影响 CORE。验证见同一迭代日志。
- [x] **Microcompact 强制 status=ok 且仅跳过 Error: 前缀** `sev-P3`
  失败结果统一规范化为 `Error:`；错误、materialization failure 与未知状态跳过
  microcompact，只有确认成功的大结果才写入 `status=ok` artifact。
- [x] **C4 迭代文档仍描述已删除的 mount_tool / tool_discovery_v1** `sev-P3`
  C4 acceptance 已明确标记为历史记录并 superseded by C5e；当前验收不得再采用
  search→mount 步骤。
- [x] **format_messages_for_compaction 不检查 tool group.complete** `sev-P3`
  formatter 现在二次守卫 `group.complete`，incomplete group 保留原始消息；新增回归测试。
