...
### 5.4 决策记录（2026-06-11）：Swarm 暂缓，不作为 agent-diva 参考路径

经过与产品负责人的讨论，明确了以下决策：

**结论：agent-diva 不走 OpenHarness Swarm 路径。**

**理由**：

1. **协作效率考量**：Swarm 的"头脑特工队"式多 agent 平等协商，消息往返成本高、容易产生竞争。Supervisor 模式（一主多子 + 任务总线）更适合 agent-diva 的"协作层尽可能高效"的产品定位。
2. **冲突解决成本**：OpenHarness Swarm 的核心价值是 Git worktree 隔离（多个 agent 改同一仓库不冲突），但需要为每个 agent 维护独立 checkout，资源开销大。agent-diva 当前的 mask 系统 Epic 3 正在用 supervisor 模式解决并行问题，成本更低。
3. **Hermes Kanban 的替代证据**：Hermes 的多 agent 协作走的是持久化任务总线（SQLite-backed Kanban），不是 mailbox-style swarm。这进一步验证 supervisor + 任务总线是更现代的范式。
4. **测试体系仍可借鉴**：本调研关注的是 OpenHarness 的**测试方法论**（事件驱动、E2E 真实 API、副作用验证），这些不依赖 swarm 实现，可被 agent-diva 完整继承。

**行动**：

- ✅ Swarm 模块（`coordinator/`、`swarm/`）仅作为**测试方法论参考**，不作为代码或架构参考
- ✅ 优先完成 mask 系统 Epic 3（supervisor 模式 + 并行 sub-agent）
- ⏸️ 不引入 Git worktree 隔离（除非 supervisor 模式在生产中出现明确冲突）
- ⏸️ 不引入 Agent 定义 frontmatter（当前 mask 的 `MaskConfig` 已是等价物）

**记录位置**：`docs/dev/awesomeagents/swarm-branch-integration-feasibility.md`、`docs/dev/awesomeagents/harness-landscape-2026.md`
