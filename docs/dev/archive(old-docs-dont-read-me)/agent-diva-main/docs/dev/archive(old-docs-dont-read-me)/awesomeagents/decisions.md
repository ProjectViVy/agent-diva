...
- 在 `agent-diva-pro` 分支主体开发完成前，**允许先交付一个不追求最终视觉表现、但强调可用性和信息清晰度的过渡界面**。
- 该过渡界面的目标是：
  - 承载新功能，尽快验证真实使用路径；
  - 暴露状态、错误、阻塞和待处理信息；
  - 为后续并入 `agent-diva-pro` 的正式视觉和交互体系提供低成本试验场。
- 该过渡界面**不承担最终品牌表达和完整设计语言统一**，避免在 `pro` 主体尚未稳定前做高成本视觉返工。
- 过渡界面的最低标准：
  - 信息层级清楚；
  - 当前状态可见；
  - 操作路径短；
  - 错误/阻塞/待处理状态明确；
  - 不制造错误心智模型。
- 结论：**宁可先做“稍丑但清楚、能用、可验证”的界面，也不为了好看推迟新功能落地。**
