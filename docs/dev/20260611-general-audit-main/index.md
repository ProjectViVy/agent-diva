...
本文档索引基于 2026-06-11 对 main 分支源码的只读分析，覆盖从 agent-diva-pro 审计报告中归属为 main-first 的 9 个问题。
...
5. P1-7 S3 todo：移除 panic 占位或完成后端实现。
6. P3-15 ProviderError 结构化：为 provider 错误处理和 UI 提示打基础。
7. P3-14 LiteLLM 拆分：在错误结构化后拆 provider 文件更顺。
8. P3-13 handlers 测试：为控制面后续拆分和修复建立回归保护。
9. P3-11 App.vue 拆分：最后做 GUI 结构性整理，降低与功能修复冲突。
