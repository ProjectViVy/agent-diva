...
///
/// This test reads `scripts/e2e/scenarios/basic_math.yaml`, runs it through
/// ScenarioRunner, and asserts that the scenario passes.
///
/// When DEEPSEEK_API_KEY is not set, the test skips gracefully (no failure).
#[tokio::test]
async fn test_basic_math_scenario() {
    if !E2EConfig::is_api_key_available() {
        eprintln!("SKIP: basic_math_test - DEEPSEEK_API_KEY not set");
        return;
    }
    ...
    let scenario_path = crate_dir
        ...
        .join("e2e")
        .join("scenarios")
        .join("basic_math.yaml");

    assert!(
        scenario_path.exists(),
        "basic_math.yaml should exist at {:?}",
        scenario_path
    );
    ...
    match result {
        Ok(Ok(scenario_result)) => {
            assert!(
                ...
                "basic_math should pass. Assertions: {:?}",
                scenario_result
                    .assertions
                    .iter()
