/** Message role for Diva Mate display */
...
/** Mate configuration shape */
export interface MateConfig {
  /** Master switch for Diva Mate sidebar entry */
  enabled: boolean
  ...
  // ===== Phase 1 新增: VRM 交互配置 =====
  /** Whether auto-hide on hover is enabled (desktop-mate mode) */
  vrmAutoHideEnabled: boolean
  /** Whether look-at tracking is enabled */
  vrmLookAtEnabled: boolean

  // ===== Desktop Mate 菜单增强 =====
  /** Desktop mate scale factor (0.75 - 1.6) */
  desktopMateScale: number
  /** Whether desktop mate window is always-on-top */
  desktopMateAlwaysOnTop: boolean
...
/** Default mate configuration */
export const DEFAULT_MATE_CONFIG: MateConfig = {
  enabled: true,
  vrmModel: '',
  ttsEnabled: false,
  asrEnabled: true,
 ...
 */
export function getTtsApiKey(config: MateConfig): string | null {
