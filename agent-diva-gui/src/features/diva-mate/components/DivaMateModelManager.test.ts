...
const { mockConfig, mockUpdateConfig } = vi.hoisted(() => {
  const mockConfig = { __v_isRef: true, value: null as unknown as MateConfig }
  const mockUpdateConfig = vi.fn((patch: Partial<MateConfig>) => {
...
describe('DivaMateModelManager', () => {
  ...
  it('uses three tabs and keeps VRM import on the VRM model tab only', async () => {
    const { default: DivaMateModelManager } = await import('./DivaMateModelManager.vue')
    const wrapper = mount(DivaMateModelManager, {
      props: { visible: true },
      global: {
