...
const appearanceApi = useAppearanceConfig(mateConfig, updateConfig)
...
const activeModelId = computed(() => toVrmModelId(mateConfig.value.vrmModel || DEFAULT_VRM_MODEL_PATH))
const modelLoaded = computed(() => !!mateConfig.value.vrmModel || models.value.length > 0)
const currentModelName = computed(() => {
  const current = models.value.find(
    (model) => model.path === mateConfig.value.vrmModel || model.id === activeModelId.value,
  )
...
async function loadModels() {
  ...
  try {
    ...
    if (inv) {
      models.value = await inv('mate_list_vrm_models') as VrmModelInfo[]
    } else {
...
function ensureEffectiveAppearance() {
  const effective = resolveAppearance(
    mateConfig.value.vrmAppearances,
    mateConfig.value.activeAppearanceId,
    models.value,
  )

  if (
    mateConfig.value.activeAppearanceId !== effective.id ||
    mateConfig.value.vrmModel !== effective.modelId
  ) {
...
async function importSelectedFile(event: Event) {
  ...
  try {
    ...
    const imported = await inv('mate_import_vrm_model', {
...
async function deleteModel(model: VrmModelInfo) {
  ...
  const inv = await getInvoke()
  if (!inv) return
  await inv('mate_delete_vrm_model', { relativePath: model.path })
  if (mateConfig.value.vrmModel === model.path) {
...
function deleteAppearance(id: string) {
  ...
  if (mateConfig.value.activeAppearanceId === DEFAULT_APPEARANCE_ID) {
    applyAppearance(DEFAULT_VRM_APPEARANCE)
  }
}

function switchAppearance(id: string) {
  const appearance = resolveAppearance(mateConfig.value.vrmAppearances, id, models.value)
...
watch(
  () => [mateConfig.value.activeAppearanceId, mateConfig.value.vrmAppearances, models.value] as const,
  () => ensureEffectiveAppearance(),
  { deep: true },
)

watch(
  () => mateConfig.value.vrmMotionList.length,
  (length) => {
    if (length === 0) {
      void loadMotions()
    }
      ...
      >
        ...
        <div class="flex-1 min-h-0 overflow-y-auto">
          <VrmAppearancePanel
            ...
            :inline="true"
            :visible="true"
            :appearances="mateConfig.vrmAppearances"
            :active-appearance-id="mateConfig.activeAppearanceId"
            :models="models"
            :motion-list="mateConfig.vrmMotionList"
          ...
          <VrmAnimationPanel
            ...
            :vrm-motion-enabled="mateConfig.vrmMotionEnabled"
