...
vi.mock('../vrm/components/DivaVrmAvatar.vue', () => ({
  default: {
    ...
    props: ['modelPath', 'mood', 'isSpeaking', 'desktopMate', 'active', 'backgroundScene', 'backgroundSceneUrl', 'idleMotionEnabled', 'selectedMotionIds', 'startMotionId', 'startMotionToken'],
    emits: [],
    setup() {
...
// ── Mate-config mock with reactive config ─────────────────────────

function makeMockMateConfig(): MateConfig {
  return {
    ...DEFAULT_MATE_CONFIG,
    desktopMateScale: 1.0,
    desktopMateAlwaysOnTop: true,
...
async function setup() {
  ...
  const { default: DesktopMateOverlay } = await import(
    './DesktopMateOverlay.vue'
  )

  const wrapper = mount(DesktopMateOverlay, {
    global: {
      stubs: {
...
async function openMenu(
  ...
  await wrapper.find('.desktop-mate-overlay').trigger('contextmenu', {
    clientX: x,
    clientY: y,
  })
  await nextTick()
...
describe('DesktopMateOverlay', () => {
  beforeEach(() => {
    mockInvoke.mockResolvedValue(undefined)
    mockListen.mockResolvedValue(() => {})
  })

  it('forces transparent background in desktop-mate mode', async () => {
    mockConfig.value.selectedGaussSceneId = 'sea'
    const { wrapper } = await setup()

    expect(wrapper.find('.desktop-mate-overlay').classes()).not.toContain('desktop-mate-app-background')
  ...
  it('emits recognized desktop-mate voice text to the main window', async () => {
    await setup()

    await mockVoiceInputOptions.current?.onRecognizedText('  hello diva  ')

    expect(mockEmitTo).toHaveBeenCalledWith('main', 'desktop-mate-voice-message', 'hello diva')
  })
  ...
  it('keeps the desktop mate neutral when expression mapping is disabled', async () => {
    const { wrapper, registeredListeners } = await setup()
  ...
  describe('click hides context menu', () => {
    it('hides context menu on overlay click', async () => {
      ...
      await wrapper.find('.desktop-mate-overlay').trigger('click')
      await nextTick()
      await nextTick()
  ...
  describe('toggle pass-through', () => {
    it('calls invoke with set_desktop_mate_ignore_mouse on toggle', async () => {
      const { wrapper } = await setup()
      await openMenu(wrapper)
  ...
  describe('toggle always-on-top', () => {
    it('calls invoke with set_desktop_mate_always_on_top', async () => {
      const { wrapper } = await setup()
      await openMenu(wrapper)
      ...
      expect(mockInvoke).toHaveBeenCalledWith(
        'set_desktop_mate_always_on_top',
  ...
  describe('toggle always-on-top failure', () => {
    it('rolls back isAlwaysOnTop state on invoke failure', async () => {
      ...
      expect(alwaysOnTopItem.find('.menu-toggle-state.on').exists()).toBe(true)
      expect(mockConfig.value.desktopMateAlwaysOnTop).toBe(true)
    })
  })
  ...
  describe('scale slider', () => {
    it('updates desktopMateScale and persists to config on input', async () => {
      const { wrapper } = await setup()
      await openMenu(wrapper)
      ...
      expect(mockConfig.value.desktopMateScale).toBe(1.25)
    })
  })
  ...
  describe('close mate', () => {
    it('calls invoke with close_desktop_mate', async () => {
      const { wrapper } = await setup()
      await openMenu(wrapper)
      ...
      expect(mockInvoke).toHaveBeenCalledWith('close_desktop_mate')
    })
  })

  // ── 10. Minimize mate ──────────────────────────────────────────

  describe('minimize mate', () => {
    it('calls invoke with minimize_desktop_mate', async () => {
      const { wrapper } = await setup()
      await openMenu(wrapper)
      ...
      expect(mockInvoke).toHaveBeenCalledWith('minimize_desktop_mate')
    })
  })
