...
type EmbeddedMateMessage =
  | {
      type: 'diva-embedded-mate:state'
      payload: {
        modelPath: string
...
function applyState(payload: EmbeddedMateMessage['payload']): void {
...
function onMessage(event: MessageEvent<EmbeddedMateMessage>): void {
  if (event.data?.type !== 'diva-embedded-mate:state') return
  applyState(event.data.payload)
}

onMounted(() => {
  window.addEventListener('message', onMessage)
  window.parent?.postMessage({ type: 'diva-embedded-mate:ready' }, '*')
})
