...
import { ref } from 'vue'
import { DEFAULT_MATE_CONFIG, type MateConfig, type VrmAppearanceConfig } from '../types'
import {
  useAppearanceConfig,
...
function setup() {
  const config = ref<MateConfig>({ ...DEFAULT_MATE_CONFIG })
  const updateConfig = vi.fn((patch: Partial<MateConfig>) => {
...
describe('useAppearanceConfig', () => {
  ...
  describe('deleteAppearance', () => {
    ...
    it('resets to default when deleting the last appearance', () => {
      ...
      expect(api.activeId.value).toBe(DEFAULT_MATE_CONFIG.activeAppearanceId)
    })
  ...
  describe('activeId reactive', () => {
    it('starts with DEFAULT_MATE_CONFIG.activeAppearanceId', () => {
      const { api } = setup()
      expect(api.activeId.value).toBe('default')
    })
