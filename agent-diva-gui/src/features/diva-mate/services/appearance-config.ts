 ...
 * appearance.  Works with MateConfig to persist appearance state.
 *
 * Adapted from super-agent-party's VRMConfig name-preset system.
 */

import { computed, type Ref } from 'vue'
import type { MateConfig, VrmAppearanceConfig } from '../types'
import {
 ...
 *
 * @param config  - Reactive MateConfig ref from useMateConfig()
 * @param updateConfig  - MateConfig patcher from useMateConfig()
 */
export function useAppearanceConfig(
  config: Ref<MateConfig>,
  updateConfig: (patch: Partial<MateConfig>) => void,
): AppearanceConfigApi {
...
function computedAppearances(config: Ref<MateConfig>) {
  return computed(() => config.value.vrmAppearances)
}

function computedActiveId(config: Ref<MateConfig>) {
