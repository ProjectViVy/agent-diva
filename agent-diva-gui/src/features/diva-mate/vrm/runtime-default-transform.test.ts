...
import {
  DESKTOP_MATE_DEFAULT_TRANSFORM,
...
describe('avatar runtime default transform', () => {
  it('uses a front-facing level camera for desktop mate mode', () => {
    const transform = getDefaultTransform('desktop-mate')
    const target = getCameraTarget(transform)
    const position = getCameraPosition(transform)

    expect(transform).toEqual(DESKTOP_MATE_DEFAULT_TRANSFORM)
    expectClose(target.x, DEFAULT_CAMERA_TARGET_X + DESKTOP_MATE_DEFAULT_TRANSFORM.offsetX)
    expectClose(target.y, DEFAULT_CAMERA_TARGET_Y + DESKTOP_MATE_DEFAULT_TRANSFORM.offsetY)
    expectClose(target.z, 0)
    expectClose(position.x, DEFAULT_CAMERA_TARGET_X + DESKTOP_MATE_DEFAULT_TRANSFORM.offsetX)
    expectClose(position.y, DEFAULT_CAMERA_TARGET_Y + DESKTOP_MATE_DEFAULT_TRANSFORM.offsetY)
  ...
  it('resets transform controller to the mode-specific default', () => {
    ...
    const controller = new TransformController(camera, controls, 'desktop-mate')

    controller.setTransform({ offsetX: 0.8, offsetY: 0.4, scale: 1.4, rotationPolar: 1.1 })
    const reset = controller.reset()

    expect(reset).toEqual(DESKTOP_MATE_DEFAULT_TRANSFORM)
    expectClose(controls.target.x, DEFAULT_CAMERA_TARGET_X + DESKTOP_MATE_DEFAULT_TRANSFORM.offsetX)
    expectClose(controls.target.y, DEFAULT_CAMERA_TARGET_Y + DESKTOP_MATE_DEFAULT_TRANSFORM.offsetY)
    expectClose(camera.position.x, DEFAULT_CAMERA_TARGET_X + DESKTOP_MATE_DEFAULT_TRANSFORM.offsetX)
    expectClose(camera.position.y, DEFAULT_CAMERA_TARGET_Y + DESKTOP_MATE_DEFAULT_TRANSFORM.offsetY)
