...
import { DESKTOP_MATE_DEFAULT_TRANSFORM } from '../../../../../avatar-runtime-vrm/src/runtime/constants'
...
onMounted(async () => {
  ...
  const mode: AvatarRuntimeMode = p.desktopMate ? 'desktop-mate' : 'embedded'
  const transparent = !!p.desktopMate || !!p.transparentBackground
  const runtimeOptions = {
    mode,
    transparent,
    allowInteraction: true,
    backgroundColor: transparent ? null : '#ffffff',
    maxFps: p.desktopMate ? 24 : 60,
  }
...
function setScale(v: number) {
  ...
  if (runtime) void syncDesktopMateCamera()
}
...
async function syncDesktopMateCamera(): Promise<void> {
  if (!runtime) return
