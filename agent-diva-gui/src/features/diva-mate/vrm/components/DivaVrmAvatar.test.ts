...
describe('background scene integration', () => {
  ...
  it('supports scene loading in desktopMate mode', async () => {
    await setup({
      backgroundScene: 'home',
      backgroundSceneUrl: '/vrm/scene/home.spz',
      desktopMate: true,
...
describe('runtime initialization', () => {
  ...
  it('uses transparent desktop-mate runtime options in desktopMate mode', async () => {
    await setup({ desktopMate: true })

    const options = mockCreateRuntime.mock.lastCall?.[1] as Record<string, unknown> | undefined
    expect(options?.mode).toBe('desktop-mate')
  ...
  it('explicitly reapplies the desktop-mate front camera after model load', async () => {
    await setup({ desktopMate: true })
