...
import {
  ...
  loadMateConfigFromCore,
  resolveAsrTranscriptionConfig,
  saveMateConfigToCore,
} from './voice-api'
...
describe('voice-api ASR config', () => {
  ...
  it('loadMateConfigFromCore maps siliconflow ASR config', async () => {
    ...
    const config = await loadMateConfigFromCore()
  ...
  it('saveMateConfigToCore persists ASR fields', async () => {
    invokeMock.mockResolvedValueOnce(JSON.stringify({ mate: {} }))
    invokeMock.mockResolvedValueOnce(undefined)

    await saveMateConfigToCore({
      ...DEFAULT_MATE_CONFIG,
  ...
  it('loadMateConfigFromCore maps provider-specific TTS keys and ignores legacy shared key', async () => {
    invokeMock.mockResolvedValueOnce(JSON.stringify({
      mate: {
  ...
  it('saveMateConfigToCore persists provider-specific TTS keys and clears legacy shared key', async () => {
    invokeMock.mockResolvedValueOnce(JSON.stringify({ mate: {} }))
    invokeMock.mockResolvedValueOnce(undefined)
  ...
  it('saveMateConfigToCore does not persist frontend-only scene selection', async () => {
    invokeMock.mockResolvedValueOnce(JSON.stringify({ mate: {} }))
    invokeMock.mockResolvedValueOnce(undefined)
