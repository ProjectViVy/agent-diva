...
const ttsLabel = computed(() => tt('mate.voice.tts', '播报'))
const micLabel = computed(() => tt('mate.voice.mic', '麦克风'))
const speakingLabel = computed(() => tt('mate.voice.speaking', '播报中'))
const pushToTalkLabel = computed(() => tt('mate.voice.pushToTalk', '按住说话'))

const ttsTitle = computed(() =>
  props.ttsEnabled ? tt('mate.voice.ttsOff', '关闭播报') : tt('mate.voice.ttsOn', '开启播报'),
)

const micTitle = computed(() => {
  if (!props.isVoiceSupported) return tt('mate.voice.notSupported', '当前环境不支持语音识别')
  if (props.voiceError) return props.voiceError
  if (props.isProcessing) return tt('mate.voice.processing', '识别中...')
  if (props.isListening) return tt('mate.voice.listening', '正在聆听...')
  if (props.isVoiceEnabled) return tt('mate.voice.enabled', '点击关闭语音')
  return tt('mate.voice.disabled', '点击开启语音')
})

const stopTitle = computed(() => tt('mate.voice.stop', '停止播报'))
const pushToTalkTitle = computed(() => (
  props.isPushToTalkDisabled
    ? props.voiceError || tt('mate.voice.notSupported', '当前不可用')
