...
describe('vrm-animation-scanner', () => {
  ...
  it('builds the runtime playable motion catalog', () => {
    ...
    expect(list.find((motion) => motion.id === 'LookAround')?.kind).toBe('oneshot')
    expect(list.find((motion) => motion.id === 'Relax')?.kind).toBe('oneshot')
    expect(list.find((motion) => motion.id === 'Sleepy')?.kind).toBe('oneshot')
    expect(list.find((motion) => motion.id === 'show_full_body')?.kind).toBe('startup')
