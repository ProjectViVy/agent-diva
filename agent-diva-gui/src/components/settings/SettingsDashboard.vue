...
import { Bot, Server, MessageSquare, Globe, Info, Search, ShieldCheck, SlidersHorizontal, WandSparkles } from 'lucide-vue-next';
import { useI18n } from 'vue-i18n';

const { t } = useI18n();

const emit = defineEmits<{
  (e: 'navigate', view: 'general' | 'mcp' | 'skills' | 'providers' | 'channels' | 'network' | 'language' | 'audit' | 'about'): void;
}>();
...
const cards = computed(() => [
  ...
  { id: 'audit', icon: ShieldCheck, title: 'Audit', desc: 'Review structured gateway behavior logs', color: 'text-emerald-700', bg: 'bg-emerald-100' },
