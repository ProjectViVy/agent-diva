...
import { useMateConfig, useMateConfigSaveState } from '../../features/diva-mate/services/mate-config'
import {
  deleteVoiceFile,
  ...
  transcribeAudio,
  type VoiceOption,
} from '../../features/diva-mate/voice/services/voice-api'
import { addVoiceLogEvent, clearVoiceLogEvents, useVoiceLog } from '../../features/diva-mate/voice/services/voice-log'
import { getTtsApiKey, type MateConfig } from '../../features/diva-mate/types'
import { isCloudAsrProvider, resolveAsrConfigDefaults } from '../../features/diva-mate/voice/services/asr-service'
import { ttsService, type TTSVoiceConfig } from '../../features/diva-mate/voice/services/tts-service'

const { t } = useI18n()
const { config, updateConfig } = useMateConfig()
const { isSaving, lastSaveError, lastSavedAt } = useMateConfigSaveState()
...
const currentModelName = computed(() => config.value.vrmModel || tt('mate.config.noModel', '未选择模型'))
...
function providerDefaults(provider: MateConfig['ttsProvider']) {
...
function asrProviderDefaults(provider: MateConfig['asrProvider']) {
...
function patchConfig(patch: Partial<MateConfig>): void {
  updateConfig(patch)
}

function patchAsrConfig(patch: Partial<MateConfig>): void {
  updateConfig(patch)
  settingsError.value = null
}

function patchVoiceConfig(patch: Partial<MateConfig>): void {
  updateConfig(patch)
  settingsError.value = null
}

function onAsrProviderChange(provider: MateConfig['asrProvider']): void {
...
function onProviderChange(provider: MateConfig['ttsProvider']): void {
...
<template>
  <div class="p-8 fade-in max-w-3xl mx-auto">
    <div class="text-center mb-8">
      ...
      <h2 class="text-2xl font-bold text-gray-800">{{ tt('mate.config.title', 'Diva Mate 设置') }}</h2>
      <p class="text-gray-500 mt-2">{{ tt('mate.config.desc', '配置桌面数字人显示与语音交互') }}</p>
    </div>
    ...
    <section class="settings-card">
      ...
      <div class="setting-row">
        <div>
          <div class="font-medium text-gray-800">启用伙伴</div>
          <div class="mt-0.5 text-sm text-gray-500">控制 Diva Mate 入口和伙伴视图</div>
        </div>
    ...
    <section class="settings-card">
      ...
      <div class="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
        <label class="field-label">
          Provider
          <select class="field-input" :value="config.asrProvider" @change="onAsrProviderChange(($event.target as HTMLSelectElement).value as MateConfig['asrProvider'])">
    ...
    <section class="settings-card">
      ...
      <div class="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
        <label class="field-label">
          Provider
          <select class="field-input" :value="config.ttsProvider" @change="onProviderChange(($event.target as HTMLSelectElement).value as MateConfig['ttsProvider'])">
