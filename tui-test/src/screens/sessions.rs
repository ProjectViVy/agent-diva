...
//!
//! Cloned from AgentDiVA TUI screens/sessions.rs with placeholder data.
...
impl SessionsState {
    pub fn new() -> Self {
        Self {
            sessions: Self::placeholder_sessions(),
            filtered: Vec::new(),
    ...
    fn placeholder_sessions() -> Vec<SessionInfo> {
        vec![
            SessionInfo {
                ...
                message_count: 45,
                created: "2026-04-16 09:30".to_string(),
            },
            SessionInfo {
                ...
                message_count: 23,
                created: "2026-04-16 08:15".to_string(),
            },
            SessionInfo {
                ...
                message_count: 12,
                created: "2026-04-15 14:00".to_string(),
            },
        ]
    }
    ...
    pub fn handle_key(&mut self, key: KeyEvent) -> SessionsAction {
        ...
        match key.code {
            KeyCode::Esc => return SessionsAction::Back,
...
pub fn draw(f: &mut Frame, area: Rect, state: &mut SessionsState, _i18n: &Translator) {
    // Initialize filtered on first draw
    if state.filtered.is_empty() && !state.sessions.is_empty() {
        state.refilter();
    }
        ...
        let items: Vec<ListItem> = state
            ...
            .map(|&idx| {
                ...
                let id_short = if s.id.len() > 12 {
                    format!("{}\u{2026}", truncate_str(&s.id, 12))
                } else {
                    s.id.clone()
                };
