...
//!
//! Cloned from AgentDiVA TUI screens/chat.rs with placeholder functionality.
...
/// Model entry for the picker (placeholder).
#[derive(Clone)]
...
impl ChatState {
    ...
    pub fn tick(&mut self) {
        if self.active_tool.is_some() || self.thinking || self.is_streaming {
...
pub fn draw(f: &mut Frame, area: Rect, state: &mut ChatState, _i18n: &Translator) {
...
fn draw_model_picker(f: &mut Frame, area: Rect, state: &ChatState) {
        ...
        .take(visible_h)
    {
        ...
        let name_display = if name.len() > max_name && max_name > 1 {
            format!("{}\u{2026}", truncate_str(name, max_name.saturating_sub(1)))
        } else {
            name.to_string()
        };
...
fn truncate_line(s: &str, max_len: usize) -> String {
    if s.len() <= max_len {
        ...
        format!("{}\u{2026}", truncate_str(s, max_len.saturating_sub(1)))
    }
}
