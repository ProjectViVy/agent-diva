//! Color palette matching the AgentDiVA landing page design system.
//!
//! Cloned from AgentDiVA TUI theme.rs.
...
pub const ACCENT: Color = Color::Rgb(244, 114, 182); // #F472B6 — Pink accent
pub const ACCENT_DIM: Color = Color::Rgb(219, 39, 119); // #DB2777 — Pink darker
...
pub const PURPLE: Color = Color::Rgb(236, 72, 153); // #EC4899 — pink-purple decorators
...
/// Uses the translator for localized badge text.
pub fn state_badge(state: &str, i18n: &crate::i18n::Translator) -> (&'static str, Style) {
    use crate::i18n::TranslationKey;
    let lower = state.to_lowercase();
    if lower.contains("run") {
        (i18n.t(TranslationKey::BadgeRun), badge_running())
    } else if lower.contains("creat") || lower.contains("new") || lower.contains("idle") {
        (i18n.t(TranslationKey::BadgeNew), badge_created())
    } else if lower.contains("sus") || lower.contains("paus") {
        (i18n.t(TranslationKey::BadgeSus), badge_suspended())
    } else if lower.contains("term") || lower.contains("stop") || lower.contains("end") {
        (i18n.t(TranslationKey::BadgeEnd), badge_terminated())
    } else if lower.contains("err") || lower.contains("crash") || lower.contains("fail") {
        (i18n.t(TranslationKey::BadgeErr), badge_crashed())
    } else {
        (i18n.t(TranslationKey::BadgeUnknown), dim_style())
    }
}
