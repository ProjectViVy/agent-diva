...
fn workspace_create_and_list() {
    ...
    assert!(create_stdout.contains("Created workspace"), "stdout: {}", create_stdout);
    ...
    assert!(list_stdout.contains("test-ws"), "list should contain test-ws: {}", list_stdout);
}
...
fn workspace_switch_updates_config() {
    ...
    assert!(switch_stdout.contains("Switched to workspace"), "stdout: {}", switch_stdout);
    assert!(switch_stdout.contains("Restart gateway for changes to take effect"), "stdout: {}", switch_stdout);

    // Verify config was updated
    let config_content = fs::read_to_string(&config_path).unwrap();
    assert!(config_content.contains("switch-ws"), "config should contain switch-ws");
}
...
fn workspace_delete_removes_workspace() {
    ...
    assert!(delete_stdout.contains("Deleted workspace"), "stdout: {}", delete_stdout);
    ...
    assert!(list_output.status.success(), "{:?}", list_output);
    let list_stdout = String::from_utf8(list_output.stdout).unwrap();
    assert!(!list_stdout.contains("delete-ws"), "list should not contain delete-ws: {}", list_stdout);
}
