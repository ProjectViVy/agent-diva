...
- [ ] **H-6: all-targets clippy cleanup in core tests**
  During P3-12 validation, `cargo clippy -p agent-diva-core --all-targets -- -D warnings` found test-target warnings outside the requested files:
  `needless_borrow` and `unnecessary_get_then_check` in `agent-diva-core/src/session/manager.rs`, plus `field_reassign_with_default` in `agent-diva-core/src/soul/mod.rs`.
  Expected behavior: core test targets should pass all-targets clippy without warnings.
