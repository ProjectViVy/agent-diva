...
- [ ] **AgentLoop G1 coordinator slimming remains after stage extraction** G1.1–G1.5 are complete: admission/execution hydration, context preparation, provider sampling/stream collection, pre-executor policy, and final persistence/events now have explicit `turn` module owners. `process_inbound_message_inner` fell from about 1,400 lines to 865 lines, but context compaction, tool-result orchestration, and Plan report demultiplexing still keep G1.6 above the approximate 500-line review target. Continue without introducing a second runtime.
