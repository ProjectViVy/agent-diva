...
use agent_diva_autodream::{
    AutoDreamEvent, AutoDreamInputCollector, AutoDreamStorage, AutoDreamWorker,
};
...
async fn worker_without_memory_home_is_a_configuration_error() {
    ...
    assert!(
        events.iter().any(|event| {
            event.kind == "memory_home_missing"
                && event.run_id.as_deref() == Some("run-contract")
                && event.phase.as_deref() == Some("orient")
                && event.failure_code.as_deref() == Some("worker_failed")
        }),
        "missing structured MemoryHome diagnostic: {events:?}"
    );
}
